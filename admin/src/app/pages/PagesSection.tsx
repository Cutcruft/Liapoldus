...
/** Группы regex-матчера против тестового path (превью `$1 → value`). */
function matchGroups(matcher: string, testPath: string): Array<{ index: number; value: string }> {
  if (!isValidRegex(matcher) || !testPath.trim()) return [];
  let re: RegExp;
  try {
  ...
  const groups = re.exec(testPath);
  if (!groups) return [];
  return groups.slice(1).map((value, i) => ({ index: i + 1, value }));
}

/**
 * Р8 — раздел «Страницы»: реестр маршрутов (R18: композиция/биндинги уехали в
 * контент-редактор). Список роутов, создание, удаление; превью regex-групп;
 * связка роут→страница. Редактирование — в RouteEditorPage (R8 legacy).
 */
export function PagesSection() {
  ...
  const groups = matchGroups(matcher, testPath);
  ...
  return (
    <Stack pad={8} gap={4}>
      <Inline justify="between" align="center">
        <h1 className="text-xl font-medium">{t('tab.section.pages')}</h1>
        <button
          type="button"
      ...
      {formOpen && (
        <form onSubmit={create} className="rounded border border-neutral-200 bg-neutral-50 p-4">
          <Stack gap={3}>
            ...
            <Inline gap={3} align="end">
              <Field label={t('site.pages.testPath')}>
                <input
                  className={INPUT_CLASS}
              ...
              {groups.length > 0 && (
                <Stack gap={1} className="rounded border border-blue-200 bg-blue-50 p-2 text-xs text-blue-800">
                  {groups.map((g) => (
                    <p key={g.index}>
                      ${g.index} → <code>{g.value}</code>
                    </p>
                  ))}
                </Stack>
              )}
      ...
      {list.state.status === 'success' &&
        (routes.length === 0 ? (
          ...
          <EntityTable<Route>
            gridClass="grid-cols-[minmax(0,3fr)_minmax(0,5rem)_minmax(0,8rem)_minmax(0,4fr)_minmax(0,9rem)]"
            getKey={(r: Route) => r.id}
            columns={[
              {
                key: 'matcher',
              ...
              {
                key: 'page',
                label: t('site.pages.page'),
                render: (r: Route) =>
                  r.action.type === 'renderPage' ? (
