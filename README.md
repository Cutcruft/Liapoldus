...
## 32. Границы платформы и прямые интеграции
...
Go backend — control plane и небольшой BFF только для собственных сущностей платформы: контента,
ассетов, простых форм, runtime-конфигурации и публикации. Он **не** является прокси для внешних
API и не хранит их секреты. Интеграции с CRM, платежами и иными внешними API выполняются
frontend-кодом напрямую; корректный CORS, авторизация и безопасность таких вызовов — зона
ответственности разработчика компонента. Всё, что попадает в browser bundle или runtime contract,
считается публичным.

Компоненты имеют два вида: `primitive` используется разработчиком в TSX, `section` добавляется
контент-редактором в страницу. Section может импортировать только явно разрешённые primitive;
все site-импорты используют `@site/components/<id>`, каждый компонент экспортирует default.
Страница — линейный список section; выбранный site/page shell получает этот список как React
`children`. Контент и локали — одна модель: locale передаётся при чтении Content, default locale
хранится в настройках сайта.
