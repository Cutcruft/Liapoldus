...
| Сторонние npm-пакеты | Дизайн-спека | Сейчас НЕ ПОДДЕРЖИВАЮТСЯ (только 4 зафиксированных shared-бандла; site-бандл не может резолвить bare-импорты). Спека `docs/backend/dependency-service.md`: zero-node/zero-sh, npm-registry напрямую (tarball+integrity), весь биндинг в Go (вкл. замену `scripts/build-shared` на `cmd/dependency-build`), резолв на publish снапшота → lock (exact+sha512), fail на несовместимых. Фаза 1 не начата |
