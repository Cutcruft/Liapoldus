...
| Сторонние npm-пакеты | Фаза 1 + subpath-слайсы (1–2) Фазы 2 | `deps.Service` (CRUD+`ResolveLock` flat-graph, exact+sha512), registry-фетчер, диск-стор tarball-блобов (`<data>/deps`), материализация (`node_modules` из кэша) + esbuild per-dep бандлы `dist/_deps/<name>@<version>.js` (scoped `%2F`), subpath-импорты site-кода **и внутри вершинных deps** → `dist/_deps/<name>@<version>/<subpath>.js` + import-map/externals, `manifest.deps`/`manifest.externals`, `DepBuildError` для несовместимых, e2e (httptest-registry + реальный esbuild). Осталось (Фаза 2/3): вложенные версионные layout, CSS-ассеты, `cmd/dependency-build`. Спека `docs/backend/dependency-service.md` |
