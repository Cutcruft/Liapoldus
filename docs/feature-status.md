...
| Сторонние npm-пакеты | Дизайн-спека + Фаза 1 частично | Сейчас в стоке только 4 зафиксированных shared-бандла; доводятся `deps.Service` (CRUD+`ResolveLock` flat-graph, exact+sha512) — готовы domain/storage/registry/admin API/тесты. Ещё НЕ готовы: materializer+bundling (этап 4 шаг 4), `cmd/dependency-build`, полный subpath/вложенный layout (Фаза 2). Спека `docs/backend/dependency-service.md` |
