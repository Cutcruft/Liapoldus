...
| Сторонние npm-пакеты | Фаза 1 + Фаза 2 (слайсы 1–3) | `deps.Service` (CRUD + `ResolveLock` **вложенный версионный graph**: BFS граней, reuse-first-satisfying, `LockedDep{Hoisted,RequestedBy}`, exact+sha512), registry-фетчер, диск-стор tarball-блобов (`<data>/deps`), материализация (hoisted → `node_modules/<name>`, несовместимая версия → `node_modules/<parent>/node_modules/<name>`) + esbuild per-dep бандлы `dist/_deps/<name>@<version>.js` (scoped `%2F`), subpath-импорты site-кода **и внутри вершинных deps** → `dist/_deps/<name>@<version>/<subpath>.js` + import-map/externals, `manifest.deps`/`manifest.externals`, `DepBuildError` для несовместимых, legacy flat-locks без маркеров раскладываются как прежде, e2e (httptest-registry + реальный esbuild: nested-версии двух вершинных deps, legacy-lock). Осталось (Фаза 2/3): CSS-ассеты, `cmd/dependency-build`. Спека `docs/backend/dependency-service.md` |
