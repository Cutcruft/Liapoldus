...
- [x] Тесты: page-store, schema-form, debounce-save, bindings-picker, preview-reload. *(Слайс 2/3: tree-utils 8, page-store 11, SchemaForm+validateValue 5, EditorPage 6 — админка 66/66, корень 318. Слайс 3/3: +preview-store 8 (rot/coalesce/poll/error/applyEvent), dev-ws 4 (reconnect/backoff/cleanup), CanvasPreview 3, ops createBuild/getBuild/deleteSnapshot 3, EditorPage автосборка+iframe 1 — админка 85/85, корень 337.)*
...
- [x] Роуты: CRUD + валидация matcher/action. *(M2-слайс 4: ops getRoute + `routes/route-utils` — isValidRegex (try/new RegExp), validateRouteFields (matcher непустой+валидный regex / target непустой / redirect status ∈ {301,302,307,308} — REDIRECT_STATUSES), normalizeAction (redirect со status+keepQuery; serveAsset/renderPage берут цель), overlapWarnings (клиентская эвристика: точный дубль matcher → duplicate; префикс с якорями ^/$ → prefix, длина ≥ 3 — не доказывает regex-пересечение, только предупреждает); SiteRoutesPage: форма создания — live-валидация на blur + блокировка submit, redirect = select статуса + чекбокс keepQuery, амбер-предупреждения пересечений (не блокируют), список показывает status/keepQuery у redirect, колонка «Редактировать»; RouteEditorPage `/sites/:siteId/routes/:routeId` — getRoute, PUT updateRoute `{matcher, priority: Number, action}`, предупреждения пересечений с excludeId, удаление → назад. Тесты: route-utils 19, SiteRoutesPage 6, RouteEditorPage 3, ops +2 — админка 168/168, корень 420.)*
- [x] Tiptap: `@tiptap/*`, slash-меню, asset-Image, HTML round-trip. *(M2-слайс 5: deps @tiptap/react|pm|core|starter-kit + extension-link|extension-image (2.27); SchemaForm: format `richtext` у Text.text → renderRichTextField; `rich-text/` — html.ts (isEmptyHtml/htmlToText/normalizeRichHtml — пусто → стабильный `<p></p>`), commands.ts (SLASH_COMMANDS базовый набор: H1/H2/H3, ul/ol, blockquote, hr, link, image + TOOLBAR_ORDER с bold/italic/strike/code; matchSlashCommands; getTextBeforeCursor; runSlashCommand/runInlineMark; slash-текст удаляется перед применением), RichTextEditor.tsx — контролируемый компонент (value/getHTML, сетка: внешняя value не перезаписывает активный набор), тулбар+slash-поп-ап (фильтр по запросу, стрелки/Enter/Escape через кастомное Extension .slashKeymap — в Tiptap v2 нет editor.on('keydown')), link через inline URL-инпут (extendMarkRange), image через AssetPicker (src=/api/assets/{id}/file); Inspector подключает редактор через React.lazy+Suspense — Tiptap в отдельном chunk (main 410.76 kB, rich-text chunk 339.70 kB gzip 108). Дизайн-превью (design-mode) рендерит Text как raw HTML через dangerouslySetInnerHTML (решение: без санитизации). Тесты: html 4, commands 3, RichTextEditor 7 (round-trip, жирный, slash Enter/стрелки/фильтр, link, Escape), EditorPage обновлён (правка через редактор, обновлённые ожидания `<p>…`), test-setup + полифилы Range.getBoundingClientRect/getClientRects — админка 183/183, корень 435.)*
- [x] Тесты соответствующих менеджеров и Tiptap-утилит. *(см. выше: html/commands/RichTextEditor + интеграция EditorPage.)*
