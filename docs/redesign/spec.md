...
Главная (карточки сайтов + создать + системные настройки)
  └── Таб сайта (URL-синхронная вкладка, шапка с тумблером режима)
        ...
        └── Режим «Редактор»: 5 разделов (вертикальная панель слева)
              ├── 1 Компоненты   — IDE: TSX в tiptap, live-валидация, schema из TS и зависимости primitive
...
| R8a | Виды компонентов | Есть только `primitive` и `section` (в БД — `isSection: boolean`). В визуальную страницу добавляются только section. |
| R8b | Зависимости компонентов | Section импортирует только явно разрешённые primitive; primitive не импортирует компонентов платформы, section не импортирует section. Пустой allowlist означает deny-by-default. Внутрисайтовый импорт — `@site/components/<id>`, у файла обязательный default export. |
...
| R14 | База сайта | Новая entity **`site-settings`**: default locale, общий head, favicon и layout-section. У каждой страницы — визуально настраиваемые head/metadata и необязательный layout override. |
...
| Компонент (definition) | Один TSX-файл с обязательным default export и schema props; `primitive` — строительный блок кода, `section` — доступный визуальной странице шаблон. Версия = git SHA. |
...
| site-settings | Сущность настроек сайта: default locale, общие метаданные/head, favicon и layout по умолчанию. |
...
ComponentDefinition {
  ...
  isSection: boolean;  // false = primitive, true = section
  allowedPrimitiveIds?: string[]; // только section; пустой список = deny-by-default
}
```

- Хранение/версии — через существующий git-механизм (компоненты — git-модель, этап 2).
- У файла один обязательный `default export`; site-компоненты импортируются только через
  стабильный alias `@site/components/<componentId>`, а не относительными путями.
- Источник по-прежнему редактируется в tiptap code-block: live TSX-валидация и диагностики
  отображаются прямо в браузере, а schema props автоматически выводится из TypeScript-интерфейса
  и дополняется ручными настройками. Проверка импорт-политики — ещё один источник диагностики,
  не замена этих возможностей.
- Только section может быть элементом `Page.list`. Section импортирует primitive только из
  `allowedPrimitiveIds`; primitive не импортирует платформенные компоненты, section не импортирует
  section. Правило проверяется в UI, сервисе и materializer до сборки.
- Layout не является третьим видом: это section с `acceptsPageContent: true`. Runtime передаёт ей
  готовый линейный список section как `children`, поэтому layout не импортирует section.
...
SiteSettings {
  ...
  head: {
    titleTemplate?: string; description?: string; faviconAssetId?: string;
...
Page {
  // ... list
  layoutSectionId?: string; // override defaultLayoutSectionId
  head: {
    title?: string; description?: string; robots?: string; canonical?: string;
    og?: Record<string, string>; meta?: Record<string, string>;
  };
}
```

- Имя и slug не дублируются. Скалярные поля `Page.head` перекрывают site defaults;
  `meta` объединяется по ключу, сначала site, затем page.
- Шрифты/favicon — ассеты из Медиа. В layout picker показываются только section с
  `acceptsPageContent`; страница наследует site layout, если не выбран override.

### 2.5 Контент и локали (R5)

- Контент и локализация — одна сущность: `Content{ Base Fields; Translations: {locale → fields} }`,
  фоллбек `merged()`. Locale — аргумент получения контента, а не отдельная модель URL/страницы.
- `SiteSettings.defaultLocale` — язык по умолчанию в редакторе и runtime. Селектор показывает
  вариативность поверх base; отсутствующие ключи маскируются base.
...
| `renderPage` | Публичный SPA-shell активного build; route и regex-группы передаются runtime для page chunk. До отдельного слайса shell допускается лишь временная явно документированная заглушка. |
...
^/articles/([0-9]+)$       → renderPage: page_x            (SPA-shell + page_x)
```
...
**Реестр (слева — список):** фильтры `Section`/`Primitive`; каждый компонент: имя/id, тип,
версия (git SHA), изменён ли (dirty), где используется (счётчик). Действия: создать, открыть,
удалить (с подтверждением, если используется), история. Произвольных групп нет.
...
- **Панель «Структура»**: `Primitive`/`Section`. У section есть allowlist primitive (пустой =
  ничего не разрешено) и флаг `принимает содержимое страницы` для выбора как shell. У primitive
  этих настроек нет. Autocomplete предлагает только разрешённые primitive; диагностика сообщает
  о запрещённом импорте, неизвестном alias, отсутствии default export и цикле.
- **Контракт файла**: один default export; site-импорт только `@site/components/<id>`.
  Вспомогательные named exports разрешены, но не становятся отдельными визуальными компонентами.
