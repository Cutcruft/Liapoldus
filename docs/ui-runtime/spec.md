...
- **Шрифты — не рантайм**: веб-шрифты сайта приходят как статические `@font-face` в
  HTML-оболочке (backend материализатор, `manifest.FontFaces`), поэтому `DesignTokens` их
  не применяет. Рантайм использует только размеченные CSS-переменные токенов.
